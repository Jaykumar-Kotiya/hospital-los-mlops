...
if latest_metrics:
    ...
    kpi3.metric("Val MAE (days)", "—")
    kpi4.metric("Val RMSE (days)", "—")
...
with tab_explain:
    ...
    st.write(
        "Ranked by **mean absolute SHAP value** (higher = more impact on the model’s LOS prediction). "
...
with tab_docs:
    ...
    st.markdown(
        """
- ✅ **Enterprise ML workflow**: preprocessing → training → evaluation → explainability → inference UI  
- ✅ **Healthcare regression**: LOS prediction with realistic output handling (clipping)  
- ✅ **Reproducibility**: artifacts stored locally + reports versioned  
- ✅ **Explainability**: SHAP global + local insights surfaced via UI  
        """
    )
